...
export function TablaAgregarRefaccionesRelacionadas({
  producto,
  ...
  const handleAgregarRefaccionRelacionada = async (id_refaccion) => {
    setIsLoading(true);
    const res = await agregarRefaccionRelacionada(id_refaccion, producto.id);
      ...
      renderCell: (params) => (
        <Tooltip title="Agregar" placement="top" arrow>
          <IconButton
            color="primary"
            onClick={() => {
              handleAgregarRefaccionRelacionada(params.row.id);
            }}
          >
  ...
  const fetchTableData = useCallback(async () => {
    setIsLoading(true);
    const data = await getTablaAgregarRefaccionesRelacionadas(paginationModel, debounceFilterModel);
    setRows(data.rows);
