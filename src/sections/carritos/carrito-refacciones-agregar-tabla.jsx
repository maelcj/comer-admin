...
import { agregarRefaccionCarrito, getTablaRefaccionesCarrito } from 'src/actions/carritos';
...
export function CarritoRefaccionesAgregarTabla({ carrito, handleGetCarrito, setOpen }) {
  ...
  const handleAgregarRefaccionCarrito = async (id_refaccion) => {
    setIsLoading(true);
    const res = await agregarRefaccionCarrito(carrito.id, id_refaccion);
    setOpen(false);
      ...
      renderCell: (params) => (
        <Tooltip title="Agregar" placement="top" arrow>
          <IconButton
            color="primary"
            onClick={() => {
              handleAgregarRefaccionCarrito(params.row.id);
            }}
          >
  ...
  const fetchTableData = useCallback(async () => {
    setIsLoading(true);
    const data = await getTablaRefaccionesCarrito(paginationModel, debounceFilterModel);
    setRows(data.rows);
