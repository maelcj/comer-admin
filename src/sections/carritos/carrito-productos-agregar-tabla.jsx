...
import { agregarProductoCarrito, getTablaProductosCarrito } from 'src/actions/carritos';
...
export function CarritoProductosAgregarTabla({ carrito, handleGetCarrito, setOpen }) {
  ...
  const handleAgregarProductoCarrito = async (id_producto) => {
    setIsLoading(true);
    const res = await agregarProductoCarrito(carrito.id, id_producto);
    setOpen(false);
      ...
      renderCell: (params) => (
        <Tooltip title="Agregar" placement="top" arrow>
          <IconButton
            color="primary"
            onClick={() => {
              handleAgregarProductoCarrito(params.row.id);
            }}
          >
  ...
  const fetchTableData = useCallback(async () => {
    setIsLoading(true);
    const data = await getTablaProductosCarrito(paginationModel, debounceFilterModel);
    setRows(data.rows);
