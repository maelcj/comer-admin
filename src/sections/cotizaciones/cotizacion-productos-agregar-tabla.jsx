...
export function CotizacionProductosAgregarTabla({ cotizacion, handleGetCotizacion, setOpen }) {
  ...
  const handleAgregarProductoCotizacion = async (id_producto) => {
    setIsLoading(true);
    const res = await agregarProductoCotizacion(cotizacion.id, id_producto);
    setOpen(false);
      ...
      renderCell: (params) => (
        <Tooltip title="Agregar" placement="top" arrow>
          <IconButton
            color="primary"
            onClick={() => {
              handleAgregarProductoCotizacion(params.row.id);
            }}
          >
  ...
  const fetchTableData = useCallback(async () => {
    setIsLoading(true);
    const data = await getTablaAgregarProductosCotizacion(paginationModel, debounceFilterModel);
    setRows(data.rows);
