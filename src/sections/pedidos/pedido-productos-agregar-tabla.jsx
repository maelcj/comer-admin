...
import { agregarProductoPedido, getTablaAgregarProductosPedido } from 'src/actions/pedidos';
...
export function PedidoProductosAgregarTabla({ pedido, handleGetPedido, setOpen }) {
  ...
  const handleAgregarProductoPedido = async (id_producto) => {
    setIsLoading(true);
    const res = await agregarProductoPedido(pedido.id, id_producto);
    setOpen(false);
      ...
      renderCell: (params) => (
        <Tooltip title="Agregar" placement="top" arrow>
          <IconButton
            color="primary"
            onClick={() => {
              handleAgregarProductoPedido(params.row.id);
            }}
          >
  ...
  const fetchTableData = useCallback(async () => {
    setIsLoading(true);
    const data = await getTablaAgregarProductosPedido(paginationModel, debounceFilterModel);
    setRows(data.rows);
