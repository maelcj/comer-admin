...
import { agregarRefaccionPedido, getTablaAgregarRefaccionesPedido } from 'src/actions/pedidos';
...
export function PedidoRefaccionesAgregarTabla({ pedido, handleGetPedido, setOpen }) {
  ...
  const handleAgregarRefaccionPedido = async (id_refaccion) => {
    setIsLoading(true);
    const res = await agregarRefaccionPedido(pedido.id, id_refaccion);
    setOpen(false);
      ...
      renderCell: (params) => (
        <Tooltip title="Agregar" placement="top" arrow>
          <IconButton
            color="primary"
            onClick={() => {
              handleAgregarRefaccionPedido(params.row.id);
            }}
          >
  ...
  const fetchTableData = useCallback(async () => {
    setIsLoading(true);
    const data = await getTablaAgregarRefaccionesPedido(paginationModel, debounceFilterModel);
    setRows(data.rows);
